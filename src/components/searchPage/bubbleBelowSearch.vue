...
<template>
  ...
  <div v-if='customChoice' style="display: flex;flex-direction: column">
    <nav>
...
export default {
  ...
  mounted() {
    ...
    this.renderBubbleChart(this.stratifyData);
  },

  methods: {
    stratifyData(seriesData) {
      let t = this.temperature_min;
      let h = this.humidity_min;
      ...
      if(window.activeName == 'seats'){
        ...
        return d3
            ...
            .sum(function (d) {
              return (handleHumidity(d.humidity) - h) * 10;
            })
            .sort(function (a, b) {
        ...
        return d3
            ...
            .sum(function (d) {
              return (handleTemperature(d.temperature) - t) * 10;
            })
            .sort(function (a, b) {
    ...
    fetchData(){ // 要先在终端运行 node app.js
      axios.get('http://127.0.0.1/getAllClassroom').then(res=>{
        ...
        for(const i in res.data){
          ...
          let t = handleTemperature(obj.temperature);
          if(t !== -1) {
            if (t < temperature_min) {
              temperature_min = t;
            }
          }
          let h = handleHumidity(obj.humidity);
          if(h !== -1) {
            if (h < humidty_min) {
              humidty_min = h;
            }
  ...
  watch:{
    activeName(){
      ...
      this.renderBubbleChartNoFetchData(this.stratifyData);
    },
    // 及时更新
    classRoomData(){
